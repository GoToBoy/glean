...
- Pass: `today-board` now requests `/entries` with explicit collection-time bounds instead of depending on timeline-first pagination.
- Pass: read items are grouped after unread items and rendered with weaker visual treatment.
- Pass: focused tests for timestamp precedence, sorting, blank-space close interaction, and mobile route consistency were added and pass.
...
- Mitigation: focused web tests pass, web typecheck passes, and the new collection filtering is isolated to `view=today-board`.
...
- Strong for helper logic, desktop close interaction, and mobile route consistency.
